...
void EnemyBullet2::update(sf::Time dt){
    ...
    float bulletSpeed = 50.f; // Ensure this is properly initialized

    sprite.move( bulletSpeed * dt.asSeconds(), 2* bulletSpeed * dt.asSeconds()); // Move the bullet downwards
}
...
bool EnemyBullet2::isOffScreen() const {
    return sprite.getPosition().y > 800; // Assuming screen height is 600
}
